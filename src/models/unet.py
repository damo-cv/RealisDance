...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None):
        ...
        if "mid_block_type" in config and config["mid_block_type"] == "UNetMidBlock2DCrossAttn":
        ...
        if not os.path.isfile(model_file):
            is_bin_file = True
            bin_model_file = os.path.join(pretrained_model_path, WEIGHTS_NAME)
            if not os.path.isfile(bin_model_file):
                raise RuntimeError(f"{model_file} and {bin_model_file} does not exist")

        state_dict = {}
        if is_bin_file:
            f = torch.load(bin_model_file, map_location="cpu")
            for k in f.keys():
                state_dict[k] = f[k]
        else:
